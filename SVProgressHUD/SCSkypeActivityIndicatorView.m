...
- (void)_commonInit
{
    ...
    self.bubbleBorderColor = [UIColor lightGrayColor];
}
...
- (SCSkypeActivityIndicatorBubbleView *)bubbleWithTimingFunction:(CAMediaTimingFunction *)timingFunction initialScale:(CGFloat)initialScale finalScale:(CGFloat)finalScale
{
    ...
    [bubbleView setBorderColor:self.bubbleBorderColor];
...
- (void)drawRect:(CGRect)rect {
    ...
    // Circle
    CGContextAddEllipseInRect(context, CGRectMake(border, border, self.bounds.size.width - 2*border, self.bounds.size.height - 2*border));
